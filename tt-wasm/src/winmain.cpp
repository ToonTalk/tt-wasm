...
static int buffer_index = 0; // file-local message-ring cursor; `static` avoids an accidental ODR clash with utils.cpp's unrelated global `buffer_index`
...
boolean Main::MessageLoopOnce() {
	...
	{ static int n = 0; if (n < 10) { n++; printf("[tt] MsgLoopOnce paused=%d time_travel=%d frame=%ld\n", (int)paused, (int)tt_time_travel, (long)tt_frame_number); fflush(stdout); } }
#endif
      ...
      tt_mouse_up_simulated = FALSE;
		...
		if (tt_time_travel == TIME_TRAVEL_JUST_PAUSED) {
			...
			{ static int gate_prints = 0;
			  if (gate_prints < 3) { gate_prints++;
				printf("[tt] draw-gate: skip_titles=%d have_focus=%d exclusive=%d dirty=%ld<%ld subs=%d\n",
					(int)tt_skip_titles, (int)have_focus, (int)tt_exclusive_screen,
					(long)tt_dirty_min_y, (long)tt_dirty_max_y, (int)tt_screen->subtitles_showing());
				fflush(stdout); } }
#endif
         if (!tt_skip_titles &&
...
#endif
#ifdef __EMSCRIPTEN__
	// Browser: the native blocking while(TRUE) can't run — drive one frame per
	// requestAnimationFrame tick, yielding to the event loop between frames. (The
	// replay try/catch below is time-travel-replay only; a normal boot just steps
	// MessageLoopOnce each frame.)
	// Browser: drive one frame per requestAnimationFrame tick instead of the native
	// blocking while(TRUE). (Reached once win_main_initialize succeeds; the replay
	// try/catch below is time-travel-replay only.)
	printf("[tt] reached MessageLoop -- win_main_initialize completed!\n"); fflush(stdout);
	// The canvas has focus from the start: WM_SETFOCUS never arrives in the browser, and
	// both the draw gate above and flip_surfaces() (winmain.cpp:3795) require have_focus.
...
//	 tt_err_file_name = copy_string("c:\\pre_init.txt"); // new on 041003
	 printf("[tt] init: entered CmdLine=%p\n",(void*)CmdLine); fflush(stdout); BeginOLE(); // moved here on 260604 since used by both HTML dialogs and text-to-speech
	 ...
	 if (tt_err_file.fail()) { // new on 290904 for error recovery (was `== NULL`: pre-C++11 ofstream void* test)
    ...
    add_to_log_if_interesting_error(0,"win_main_initialize");
	 ...
	 tt_main_instance = hInstance; printf("[tt] init: temp-path + err-file done\n"); fflush(stdout); // new on 260603 for DynaZip - global variable existed but wasn't used
	 wait_cursor();
	 Main::hPrevInstance = hPrevInstance;
	 Main::nCmdShow = nCmdShow;
	 another_toontalk_is_running = (FindWindow("ToonTalk",NULL) != NULL);
	 HWND other_window = FindWindowG(szClassName,window_title); printf("[tt] init: 9527 after FindWindowG\n"); fflush(stdout); 
...
//    tt_original_command_line = lpszCmdLine; -- obsolete
	 ...
	 if (Main::hPrevInstance == NULL) {
		 MainWindow::Register(); printf("[tt] init: 9556 after Register\n"); fflush(stdout);
	 };
//#if !TT_NEW_IO
//    tt_temporary_buffer = new unsigned char[tt_temporary_buffer_size]; // used by pre_tt_err_file
//#endif
	 printf("[tt] 9560 CmdLine ptr=%p\n",(void*)CmdLine); fflush(stdout); printf("[tt] 9560 CmdLine[0]=%d\n", CmdLine ? (int)(unsigned char)CmdLine[0] : -99); fflush(stdout); tt_command_line_for_exe_to_run_upon_shutdown = copy_string(CmdLine); printf("[tt] init: 9561 cmdline copied\n"); fflush(stdout); // new on 021103
	 allocate_resource_strings(); printf("[tt] init: 9562 resource strings allocated\n"); fflush(stdout); // early since used by static strings as well
...
//#endif
          delete [] tt_ini_file;
          set_ini_file(copy_string(AC(IDC_TT_INI))); printf("[tt] init: 9629 ini set\n"); fflush(stdout); // use default search path
       };
    };
#if TT_DEBUG_ON
...
#endif
	 set_directory_names(); printf("[tt] 9640 dirs set\n"); fflush(stdout); // new on 300500
#if TT_DEBUG_ON
    ...
    if (tt_language == NO_LANGUAGE_SPECIFIED) {
       int language_answer = default_setting(AC(IDC_LANGUAGE)); printf("[tt] 9657 lang_answer=%d\n",language_answer); fflush(stdout);
       ...
       ascii_string default_language = ini_entry(AC(IDC_VERSIONS),language_key,FALSE); printf("[tt] 9665 default_language read\n"); fflush(stdout);
 #if TT_DEBUG_ON
		add_to_log_if_interesting_error();
#endif
    ...
    add_to_log_if_interesting_error();
#endif
    load_string_library(); printf("[tt] 9681 strlib loaded\n"); fflush(stdout); // no-op if done; otherwise default
//#if TT_32
...
//#endif
   ...
   write_ini_entry(AC(IDC_SWITCHES),AC(IDC_TOONTALK_STARTED),"1"); printf("[tt] 9689 wrote started flag\n"); fflush(stdout);
...
//   tt_main_window = &MainWnd;
	tt_main_window = new MainWindow(); printf("[tt] 9693 MainWindow created\n"); fflush(stdout);
...
//		tt_error_file() << (int) tt_joystick_pictures[i]->kind_of() << " " 
	...
	initialize_unzip();
#endif
	interpret_command_line(command_count,commands); printf("[tt] 9725 cmdline interpreted\n"); fflush(stdout); //,TRUE);
	...
	tt_bits_per_pixel = GetDeviceCaps(visible_device_context,BITSPIXEL); printf("[tt] 9739 bits_per_pixel=%d\n", tt_bits_per_pixel); fflush(stdout);  // new on 220901
...
//	tt_install_current_exe = ini_int("Switches","InstallCurrentExe",FALSE,tt_install_current_exe); // new on 020304
	...
	if (tt_fixed_font_name == NULL) { 
	   ...
	   if (need_wide_characters()) {
			...
			tt_fixed_font_name = copy_ascii_string("�l�r �o�S�V�b�N");
#else
   ...
   if (tt_variable_font_name == NULL) {
	   if (need_wide_characters()) {
#if TT_FOR_FAR_EAST_WINDOWS
			tt_variable_font_name = copy_ascii_string("�l�r �o�S�V�b�N");
#else
...
// also tried standard_exit(TRUE); // but this is a bad time to cleanup since only partially initialized
	...
	window_title = new_title_for_window(tt_user_name); printf("[tt] 9947 reached (approaching tt_initialize)\n"); fflush(stdout); // updated on 221103
#if TT_SPECIAL_VERSION_TO_DEBUG_WIN_ME
...
#endif
 	printf("[tt] tt_initialize SUCCESS -> tt_initialize_objects\n"); fflush(stdout); tt_initialize_objects();
#if TT_SPECIAL_VERSION_TO_DEBUG_WIN_ME
	tt_error_file() << "Finished tt_initialize_objects" << endl;
#endif
	initialize_background_sprites(); printf("[tt] 9981 bg sprites done (tt_initialize completed!)\n"); fflush(stdout); // moved here on 250901
#if TT_SPECIAL_VERSION_TO_DEBUG_WIN_ME
...
void copy_screen_to_file(const_ascii_string file_name, int min_x, int max_x, int min_y, int max_y) {
	...
	if (file.fail()) { // was `== NULL`: pre-C++11 ofstream void* test
