...
//            || (tt_language == SWEDISH && (part[i] == 'i' || part[i] == '�' || part[i] == '�'))) &&
            (i == 0 || part[i-1] == ' ') &&
...
//      case SWEDISH:
//         {
//         string extras[] = {"Oj d�","Jag kan tyv�rr inte komma ih�g vad jag skulle s�ga",
//                            "n�got fel", "tyv�rr kan", "tar bort", "eller s� �r en bildfil d�lig", // to maintain fewest changes to phrase.ini
//                            "�","�","�","�","�","�",NULL};
//         int i = 0;
//         while (extras[i] != NULL) {
...
//	boolean changing_names = FALSE;
   ...
   tt_err_file.open(tt_err_file_name,flags);
	if (!tt_err_file) { // new on 290904 for error recovery -- old MSVC `== NULL` (operator void*) -> operator bool
...
boolean tt_initialize() {
   ...
   tt_temp_files = new ascii_string[tt_temp_files_max]; printf("[tt] tt_init: entered\n"); fflush(stdout); // obsolete??
   open_tt_error_file();
...
//   true_previous_time = previous_time; // commented out on 150204 since gives wrong delta for first cycle
   ...
   printf("[tt] tt_init: pre tt_setmode(%d)\n", tt_video_mode); fflush(stdout);
   if (!tt_setmode(tt_video_mode,FALSE)) { printf("[tt] tt_init: tt_setmode FALSE\n"); fflush(stdout); return(FALSE); };
   printf("[tt] tt_init: tt_setmode OK\n"); fflush(stdout);
...
//	tt_err_file.close(); // moved here from below on 290999 to avoid code guard errors about fflush
	...
	boolean show_log = // moved here on 100999 since used to crash NT and Win 2000
				(tt_err_file_name != NULL && !tt_temporary_user && 
				 ...
				 !tt_error_file().fail() && // new on 300904 so don't offer to show if couldn't create it -- updated to use tt_error_file() on 211004; old MSVC `!= NULL` (operator void*) -> !fail()
