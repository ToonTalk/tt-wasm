...
// WASM port: the original `#import "msxml4.dll"` read the MSXML4 type library to
// generate the MSXML2 smart-pointer wrappers. clang/emcc has no #import, and
// this file uses the raw IXMLDOM* COM interfaces directly (the `using namespace
// MSXML2;` was already commented out), so we substitute the opaque interface
// shim. The DOM is reimplemented over a real parser in phase 1.
#include <msxml.h>
...
xml_element *xml_node_to_element(xml_node *node) { // new on 190703
	...
	xml_element *element;
	node->QueryInterface(IID_IXMLDOMElement, (void **) &element); // was __uuidof(IXMLDOMElement); same IID, but the shim attaches no uuid attribute
   return(element);
};
...
xml_document *document_from_file(string file_name) {
   ...
   DWORD start;
	...
	if (file.fail()) { // new on 040903 to check for 0 byte files (was `== NULL`: pre-C++11 ifstream void* test)
