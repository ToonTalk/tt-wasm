...
// On 32-bit MSVC `long` and `int` were the same width and `long` arguments
// (e.g. city_coordinate) bound to the int overload; modern clang finds the
// long->int / long->LONGLONG / long->double conversions equally ranked and so
// reports ambiguity. Restore the original resolution with an exact-match long
// overload that forwards to the int overload (behavior-identical).
inline boolean xml_set_attribute(xml_element *element, wide_string name, long value) {
	return xml_set_attribute(element, name, (int) value);
}
