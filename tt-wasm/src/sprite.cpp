...
//		delete [] buffer; // commented out on 021001 since used by multiple callers now
	...
	int version;
	file >> version;
	printf("[tt] load_sprite code=%d tok0='%s' ver=%d\n", (int)code, temp_string, version); fflush(stdout);
	if (version != 1) {
//#if !TT_WINDOWS
//		file.close();
//#endif
...
//					images[j].set_file_name(NULL);
				...
				file.close();
#endif
				printf("[tt] load_sprite code=%d NOT-BMP temp_string='%s'\n", (int)code, temp_string); fflush(stdout);
...
boolean load_sprite_file(SpriteCode code) {
	if (tt_builtin_picture_directory != NULL) { 
		...
		if (file) { // old MSVC `!= NULL` (operator void*) -> operator bool
...
//  };
  ...
  delete [] sprite_offsets; printf("[tt] sprite_values: 69-sprite load loop done\n"); fflush(stdout); // done with it now
...
//		file.put('\0');
	...
	printf("[tt] open_images: opened '%s' fd=%d status=%d cookie=%c%c\n", file_name, (int)images_file, status, cookie[0], cookie[1]); fflush(stdout);
