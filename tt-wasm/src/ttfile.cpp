...
#include "strings.h" // WASM port: was the absolute path "c:\tt\strings.h"; the header lives in the source tree (resolved via -I src)
				...
				if (types[index] == ']') { // OK so far
					if (types[index+1] == 'B' && types[index+2] == ']') { // everything fine
						...
						for (int i = 0; i < index; i++) {
							buffer[i] = (unsigned char)(long) data[i+1]; // pointer slot holds an int value (cf. (long)data[1] above); take its low byte
						};
                  DWORD number_of_bytes_written;
