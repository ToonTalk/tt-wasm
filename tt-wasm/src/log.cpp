...
//			log_out.close(); // experiment 190703
			...
			if (!log_out) { // new on 290904 for error recovery -- old MSVC `== NULL` (operator void*) -> operator bool
...
void get_all_subtitles() { // new on 271003 - read all the entire file and store it so can jump in time
	...
	short int length;
	while (!subtitles_file.fail()) { // was `!= NULL`: pre-C++11 ifstream void* test
...
//   delete [] subtitles_file_name;
   if (subtitles_file.fail()) return(FALSE); // was `== NULL`: pre-C++11 ifstream void* test
...
//			if (speak_file != -1) {
		...
		if (tt_debug_mode == 1540) { // read start times from ascii file named start.log (not a documented feature)
         ...
         for (short int i = 0; i < max_speak_count; i++) {
            if (!start_times.fail()) { // was `!= NULL`: pre-C++11 ifstream void* test
