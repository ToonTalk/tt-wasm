...
//#endif
  ...
  printf("[tt] setmode: screen_parameters OK\n"); fflush(stdout);
...
//	  MessageBox(NULL,"about to reallocate_appearance_data","TT",MB_OK); // just for testing!! 270105
	  reallocate_appearance_data(); printf("[tt] setmode: reallocate_appearance_data OK\n"); fflush(stdout); // since changing resolution -- new on 240105
  };
  if (!open_images_file(video_mode)) return(FALSE);
  printf("[tt] setmode: open_images_file OK\n"); fflush(stdout);
  mode_set = TRUE;
...
//#endif
  printf("[tt] setmode: pre initialize_sprite_values\n"); fflush(stdout);
  if (!initialize_sprite_values()) return(FALSE);
  printf("[tt] setmode: initialize_sprite_values OK\n"); fflush(stdout);
...
void dump_history(boolean puzzle_state_changed) {
   ...
   output_file_stream history_file(full_file_name,std::ios_base::binary|std::ios_base::out);
	if (!history_file) { // new on 290904 for error recovery -- old MSVC `== NULL` (operator void*) -> operator bool
...
//    long number = atoi(text);
      ...
      stream >> number;
      boolean non_number = stream.fail(); // didn't read a number -- old MSVC `== NULL` (operator void*) -> fail()
      // if there is some non-white space characters left it's not a number
      while (TRUE) {
         int ch = stream.get();
         if (ch == EOF || stream.fail()) break;
         if (ch != ' ' && ch != '\r' && ch != '\t' && ch != '\n') {
            non_number = TRUE;
            break;
         };
...
void save_clipping_to_file_name(HGLOBAL clipping, string file_name) {
	...
	if (tt_zip_files) { // new on 180303 to package everything up and zip the XML
		...
		if (copy.fail()) { // new on 290904 for error recovery (was `== NULL`: pre-C++11 ofstream void* test)
...
string read_string(input_stream &in, string s) {
	if (in.fail()) return(NULL); // new on 050600 (was `== NULL`: pre-C++11 istream void* test)
...
#endif
		if (in.fail()) { // new on 180903 to deal with broken files (or code) (was `== NULL`: pre-C++11 istream void* test)
			delete [] s;
...
//	DeleteFile(temp_file);
	...
	if (!truncated_file) { // new on 290904 for error recovery -- old MSVC `== NULL` (operator void*) -> operator bool
		unable_to_write_file(file_name);
		return;
	};
...
output_stream & tt_error_file() { // new on 300904
	if (tt_err_file.fail()) { // was `== NULL`: pre-C++11 ofstream void* test
