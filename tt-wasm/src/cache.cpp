...
DeletedMemoryDrawSurface::~DeletedMemoryDrawSurface() {
	...
	// Port tolerance: with synthetic art some surface slots are NULL where the
	// native build always had real surfaces; Release() through NULL is a wasm trap.
	if (memory == NULL) { printf("[tt] ~DeletedMemoryDrawSurface: NULL surface, skipping Release\n"); fflush(stdout); return; }
#endif
#if TT_DEBUG_ON
