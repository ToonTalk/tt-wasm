...
void copy_work_page(work_page source, work_page destination,
	...
	{ static int n = 0; if (n < 8) { n++;
	  long snz = 0; for (int i = 0; i < width_in_bytes; i++) if (((unsigned char*)source)[i]) snz++;
	  printf("[tt] copy_work_page dst=%p src=%p %ldx%ld dpitch=%ld src_nonzero_row0=%ld\n",
	    (void*)destination, (void*)source, (long)width, (long)height, (long)destination_pitch, snz); fflush(stdout); } }
#endif
	for (int y = 0; y < height-1; y++) {
