...
and finally, so the website pages are printable too, let Word convert the sheets this script
just wrote (it opens .htm quite happily -- the guide comes out clean):

    $dir = "<repo>\\web\\infinity\\sheets"
    foreach ($p in @("guidance.htm","guidance2.htm","historical_note.htm",
                     "resort_infinity_guide.htm","diagonal.htm","no_copies.htm",
                     "notebook.htm","resort_infinity.htm")) {
      $doc = $word.Documents.Open((Join-Path $dir $p), $false, $true)
      $doc.ExportAsFixedFormat((Join-Path "$dir\\pdf" ($p -replace '\\.htm$','.pdf')),
                               17, $false, 0, 0, 0, 0, 0, $true, $true, 0, $true, $true, $false)
      $doc.Close(0)
    }

then re-run this script, which regenerates sheets/pdfs.js from whatever PDFs now exist.
...
<p>The ready-made solution box &mdash; the page's <b>Answer to problem 1</b> button has Tooly
bring it to the floor. <b>Its Guests hole is empty on purpose:</b> before giving the box to the
...
def write_pdf_index():
    """Record which sheets have a printed page, by looking.

    The page used to decide this from which GROUP a sheet was listed in, in two places that
    then disagreed -- Resort Infinity moved groups and started serving a 404 (Ken, 2026-08-12).
    A generated list cannot drift from the files.
    """
    pdfdir = os.path.join(DST, "pdf")
    have = set(f[:-4] for f in os.listdir(pdfdir)) if os.path.isdir(pdfdir) else set()
    names = sorted(f for f in os.listdir(DST) if f.endswith(".htm") and f[:-4] in have)
    with open(os.path.join(DST, "pdfs.js"), "w", encoding="utf-8") as f:
        f.write("// GENERATED by build-sheets.py -- which sheets have a printed page.\n")
        f.write("var SHEET_PDFS = {\n")
        for nm in names:
            f.write('  %s: 1,\n' % repr(nm).replace("'", '"'))
        f.write("};\n")
    print("pdfs.js: %d of %d sheets have a printed page" %
          (len(names), len([f for f in os.listdir(DST) if f.endswith(".htm")])))
